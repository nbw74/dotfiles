...
func! SetFileTypeSH(name)
  ...
  elseif exists("g:bash_is_sh") || a:name =~ '\<bash\>' || a:name =~ '\<bash2\>' || a:name =~ '\<vbash\>'
    let b:is_bash = 1
...
" Crontab
au BufNewFile,BufRead .crontab*,crontab,crontab.*,/etc/cron.d/*		call s:StarSetf('crontab.freebsd')
