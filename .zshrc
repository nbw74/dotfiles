...
fpath=(~/.zsh/completion /usr/share/zsh/$ZSH_VERSION/functions $fpath)	# Where to look for autoloaded function definitions
...
alias info_pg_replication='[[ $(psql -Upostgres -AXtc "SELECT pg_is_in_recovery()") == "t" ]] && \
    ...
    psql -Upostgres -Xc "SELECT client_addr, state, sent_lsn, write_lsn, flush_lsn, replay_lsn FROM pg_stat_replication;"; \
    else \
    psql -Upostgres -Xc "SELECT client_addr, state, sent_location, write_location, flush_location, replay_location FROM pg_stat_replication;"; fi'
